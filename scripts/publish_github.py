...
def git(*args: str, check=True) -> str:
    return run(GIT, "-c", f"safe.directory={SAFE_ROOT}", *args, check=check)
...
def main():
    ...
    changed = subprocess.run([GIT, "-c", f"safe.directory={SAFE_ROOT}", "diff", "--cached", "--quiet"], cwd=ROOT).returncode != 0
    if changed:
